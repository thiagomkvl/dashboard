...
if 'df_pagamentos' not in st.session_state:
    try:
        ...
        # =================================================================
        # 🟢 NOVA FUNCIONALIDADE: FORMATAÇÃO DE CNPJ/CPF (ZEROS À ESQUERDA)
        # =================================================================
        def corrigir_zeros_doc(valor):
            """Recupera os zeros que o Sheets/Excel removeu"""
            s_val = str(valor).strip()
            if s_val in ['nan', 'None', '', '0']: return ""
            if s_val.endswith('.0'): s_val = s_val[:-2] # Remove decimal de float
            
            limpo = ''.join(filter(str.isdigit, s_val))
            if not limpo: return ""
            
            # Lógica: Se > 11 dígitos, é CNPJ (força 14). Se <= 11, é CPF (força 11).
            if len(limpo) > 11:
        ...
        if 'cnpj_beneficiario' in df_p.columns:
            df_p['cnpj_beneficiario'] = df_p['cnpj_beneficiario'].apply(corrigir_zeros_doc)

        # 2. Aplica na Chave Pix (SOMENTE se for numérica = CPF/CNPJ)
        def corrigir_chave_pix(valor):
            s_val = str(valor).strip()
            if s_val.endswith('.0'): s_val = s_val[:-2]
            
            # Se tem @ ou traço longo (UUID), não mexe
            if '@' in s_val or (len(s_val) > 20 and '-' in s_val):
                return s_val
            
            nums = ''.join(filter(str.isdigit, s_val))
            if len(nums) >= 11: # Se parece CPF ou CNPJ
                return corrigir_zeros_doc(nums)
            return s_val
