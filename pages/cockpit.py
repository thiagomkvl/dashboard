        ...
        with col_botoes:
            ...
            if not lote_pix.empty:
                c_btn1.download_button(
                    f"Baixar Lote PIX ({len(lote_pix)})", 
                    gerar_cnab_pix(lote_pix, DADOS_HOSPITAL, nsa=nsa_hoje), # Passando o NSA
                    f"REM_PIX_{datetime.now().strftime('%d%m')}_NSA{nsa_hoje}.txt"
                )
            if not lote_boleto.empty:
                c_btn2.download_button(
                    f"Baixar Lote BOLETOS ({len(lote_boleto)})", 
                    gerar_cnab_boleto(lote_boleto, DADOS_HOSPITAL, nsa=nsa_hoje), # Passando o NSA
                    f"REM_BOLETO_{datetime.now().strftime('%d%m')}_NSA{nsa_hoje}.txt"
                )
...
if st.button("💾 Salvar Alterações"):
    ...
    st.toast("Salvo!", icon="✅")import streamlit as st
import pandas as pd
from datetime import datetime
from database import conectar_sheets
# IMPORTANDO AS FUNÇÕES DO MOTOR
