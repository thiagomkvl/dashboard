...
<style>
    ...
    /* TABELAS COM ALINHAMENTO FIXO (AUMENTADO) */
...
def limpa_valor(valor):
    try:
        ...
        if v_str in ["", "-", "nan", "None", "NaN"]: return 0.0
...
def formatar_moeda_curta(valor):
    try:
        ...
        elif abs(val) >= 1_000: return f"R$ {val/1_000:.0f}K".replace('.', ',')
        return f"R$ {val:.0f}"
    except: return "-"
...
with st.sidebar:
    ...
    if st.button("Limpar Filtros Aplicados", use_container_width=True): st.rerun()
...
if obra_selecionada != "Todas": df_real_mom = df_real_mom[df_real_mom['Obra'] == obra_selecionada]
...
if realizado_anterior > 0: mom_pct = ((realizado_atual / realizado_anterior) - 1) * 100
else: mom_pct = 100 if realizado_atual > 0 else 0

if mom_pct > 0: mom_str, mom_color = f"↗ +{mom_pct:.1f}% vs Mês {mes_anterior:02d}", "var(--red-main)"
elif mom_pct < 0: mom_str, mom_color = f"↘ {mom_pct:.1f}% vs Mês {mes_anterior:02d}", "var(--green-main)"
else: mom_str, mom_color = f"→ 0.0% vs Mês {mes_anterior:02d}", "var(--text-muted)"
...
kpi_html = (
    "<div class='kpi-grid'>"
    "<div class='kpi-item bg-blue'><div class='kpi-title'>Orçamento Total</div>" + f"<div class='kpi-value'>{formatar_moeda(total_orcado)}</div><div class='kpi-subtitle'>Valor planejado atualizado</div></div>"
    "<div class='kpi-item bg-green'><div class='kpi-title'>Investimento Realizado</div>" + f"<div class='kpi-value'>{formatar_moeda(total_realizado)}</div><div class='kpi-subtitle' style='color:{pb_color}; font-weight:600;'>{consumo_geral_perc:.1f}% do orçado</div>{html_pb}</div>"
    "<div class='kpi-item bg-yellow'><div class='kpi-title'>Orçamento Disponível</div>" + f"<div class='kpi-value' style='color: {'var(--red-main)' if saldo_orcamento < 0 else 'var(--text-dark)'};'>{formatar_moeda(saldo_orcamento)}</div><div class='kpi-subtitle'>Saldo para queima</div></div>"
...
for _, r in df_linha.iterrows(): html_unified += f"<th>{r['Mes_Nome']}</th>"
html_unified += "</tr></thead><tbody><tr><td class='row-label'>Realizado</td>"
for _, r in df_linha.iterrows(): html_unified += f"<td class='val-real'>{formatar_moeda_curta(r['Valor_Realizado']) if pd.notna(r['Valor_Realizado']) else '-'}</td>"
html_unified += "</tr><tr><td class='row-label'>Orçado</td>"
for _, r in df_linha.iterrows(): html_unified += f"<td class='val-orc'>{formatar_moeda_curta(r['Valor_Orcado'])}</td>"
...
# ==============================================================================
# 7. TABELA DETALHADA DE FASES DA OBRA (EXPORT + ORDENAÇÃO DE PESO)
# ==============================================================================
col_tit1, col_btn1 = st.columns([8, 2])
with col_tit1: st.markdown("<div class='section-title' style='margin-top:0;'>Detalhamento do Orçado x Realizado - Fases da Obra</div>", unsafe_allow_html=True)
...
if not df_fases.empty:
    ...
    # Prepara ordenação por Peso e extrai totais
    obra_totals_calc = []
    ...
    for col in cols_to_show: html_fases += f"<th style='text-align:right;'>{'ORÇAMENTO R$' if 'custo' in str(col).lower() else col}</th>"
    html_fases += "</tr></thead>"
    ...
    for col_name in cols_to_show: html_fases += f"<td style='text-align:right;'>{formatar_moeda(totais_mensais_fases[col_name])}</td>"
...
with col_tit2: st.markdown("<div class='section-title' style='margin-top:0;'>Detalhamento de Pagamentos Realizados</div>", unsafe_allow_html=True)
...
if not df_real_detalhe.empty:
    ...
    for m in range(2, 13):
        if m not in df_matrix.columns: df_matrix[m] = 0.0
    ...
    # ORDENA AS OBRAS DO MAIOR PAGAMENTO PARA O MENOR
    ...
    for m in range(2, 13):
        if m not in df_orc_pivot.columns: df_orc_pivot[m] = 0.0
    ...
    for col_m in meses_tabela: html_real += f"<th style='text-align:right;'>{col_m}</th>"
    html_real += "</tr></thead>"
    ...
    for _, row in df_matrix.iterrows():
        ...
        if tot_r > tot_orc and tot_orc > 0: color_tot = "#ef4444" # HEATMAP: Estourou orçado (Vermelho)
        elif tot_r > 0 and tot_r <= tot_orc: color_tot = "#10b981" # HEATMAP: Dentro do orçado (Verde)
        ...
        for m_num, col_m in zip(range(2, 13), meses_tabela):
            ...
            if val_m > val_orc_m and val_orc_m > 0: color_val = "#ef4444"
            elif val_m > 0 and val_m <= val_orc_m: color_val = "#10b981"
        ...
        if not df_trans_esp.empty:
            for _, tr in df_trans_esp.iterrows():
                ...
                for m_num in range(2, 13):
                    if m_num == mes_ref: html_real += f"<td style='text-align:right; font-size: 10px; font-weight:600; color: var(--text-dark);'>{formatar_moeda(valor_pagamento)}</td>"
                    else: html_real += f"<td style='text-align:right; font-size: 10px; color: #cbd5e1;'>-</td>"
                html_real += "</tr>"
        html_real += "</tbody>"
    ...
    st.info("Nenhum pagamento realizado encontrado para os filtros selecionados.")import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import re
