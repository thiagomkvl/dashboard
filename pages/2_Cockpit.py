...
# Importando explicitamente as duas funções de geração e os dados do hospital
...
if 'df_pagamentos' not in st.session_state:
    try:
        ...
        # Migração legado (se existir coluna antiga)
...
if not st.session_state['df_pagamentos'].empty:
    # Cria cópia para exibição com a coluna "Tipo" calculada na hora
    ...
    # Sincroniza edição (remove a coluna Tipo antes de salvar no state)
    ...
    if not df_pagar.empty:
        # Recalcula tipos para separar os lotes
        ...
        st.warning("Selecione pelo menos um item na tabela para liberar os botões de download.")
    ...
    st.info("A lista de pagamentos está vazia. Adicione itens manualmente ou faça upload na aba correspondente.")
