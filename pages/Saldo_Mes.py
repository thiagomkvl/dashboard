...
def carregar_dados(data_inicio, data_fim):
    ...
    try:
        ...
        # ==============================================================================
        # ADAPTAÇÃO: AGRUPAMENTO MENSAL PARA O GRÁFICO
        # ==============================================================================
        if df_extratos is not None and not df_extratos.empty:
            ...
            # Criação da coluna de Mês/Ano para agrupar
            df_ext_caixa['MesAno'] = df_ext_caixa['Data'].dt.to_period('M')
            # Data de referência para ordenar as barras
            df_ext_caixa['Data_Ref'] = df_ext_caixa['MesAno'].dt.start_time
            
            df_extratos_mensal = df_ext_caixa.groupby('Data_Ref').agg({'Vl Crédito': 'sum', 'Vl Débito': 'sum', 'Cred_Op': 'sum', 'Deb_Op': 'sum'}).reset_index()
            
            df_graficos = df_extratos_mensal.sort_values('Data_Ref').copy()
            ...
            # Formato do Label do Eixo X: Mês/Ano (ex: 01/2026)
            df_graficos['Data_Label'] = df_graficos['Data_Ref'].dt.strftime('%m/%Y')
...
with c2:
    ...
    # ADAPTAÇÃO: Título alterado para MENSAL
    st.markdown("<div class='section-title' style='margin-top:10px;'>EVOLUÇÃO MENSAL DO SALDO TOTAL</div>", unsafe_allow_html=True)
...
with c3:
    ...
    if not df_aplicacoes_nova.empty:
        ...
        for _, row in df_aplicacoes_nova.iterrows():
            ...
            # --- CÓDIGO COMPLETADO A PARTIR DAQUI ---
            atual = row[c_atual] if c_atual else 0
            
            tot_si += si; tot_app += app; tot_imp += imp; tot_rend += rend; tot_resg += resg; tot_atual += atual
            
            html_app += f'<tr><td>{banco}</td><td class="valores">{formatar_moeda(si)}</td><td class="valores" style="color:#008A8C;">{formatar_moeda(app)}</td><td class="valores" style="color:#e74a3b;">{formatar_moeda(imp)}</td><td class="valores" style="color:#1cc88a;">{formatar_moeda(rend)}</td><td class="valores" style="color:#c58a16;">{formatar_moeda(resg)}</td><td class="valores valor-destaque">{formatar_moeda(atual)}</td></tr>'
        ...
        st.info("Nenhuma movimentação de aplicação no período selecionado.")

# ==============================================================================
# TABELA CONSOLIDADA DE CAIXA (CÓDIGO ADICIONADO PARA FINALIZAR O DASHBOARD)
# ==============================================================================
st.markdown("<br><div class='section-title'>POSIÇÃO CONSOLIDADA DE CAIXA</div>", unsafe_allow_html=True)

if not df_consolidado.empty:
    html_cons = f'<div class="tabela-container-scroll"><table class="tabela-financeira"><thead><tr><th>BANCO / INSTITUIÇÃO</th><th>TIPO</th><th class="valores">SALDO INICIAL</th><th class="valores">ENTRADAS OP.</th><th class="valores">SAÍDAS OP.</th><th class="valores">ENTRADAS TR.</th><th class="valores">SAÍDAS TR.</th><th class="valores">SALDO ATUAL</th><th class="valores">LIMITE / GARANTIDA</th></tr></thead><tbody>'
    
    t_si = 0; t_eop = 0; t_sop = 0; t_etr = 0; t_str = 0; t_sf = 0; t_lim = 0
    
    for _, row in df_consolidado.iterrows():
        b = row[col_conta]
        tipo = row['Tipo']
        si = row.get('Saldo Inicial', 0)
        eop = row.get('Entrada Op', 0)
        sop = row.get('Saída Op', 0)
        etr = row.get('Entrada Tr', 0)
        str_ = row.get('Saída Tr', 0)
        sf = row.get('Saldo Final', 0)
        lim = row.get('Conta Garantida', 0)
        
        t_si += si; t_eop += eop; t_sop += sop; t_etr += etr; t_str += str_; t_sf += sf; t_lim += lim
        
        html_cons += f'<tr><td>{b}</td><td>{tipo}</td><td class="valores">{formatar_moeda(si)}</td><td class="valores" style="color:#1cc88a;">{formatar_moeda(eop)}</td><td class="valores" style="color:#e74a3b;">{formatar_moeda(sop)}</td><td class="valores">{formatar_moeda(etr)}</td><td class="valores">{formatar_moeda(str_)}</td><td class="valores valor-destaque">{formatar_moeda(sf)}</td><td class="valores" style="color:#6b7280;">{formatar_moeda(lim)}</td></tr>'
        
    html_cons += f'<tr class="linha-total"><td>TOTAL GERAL</td><td>-</td><td class="valores">{formatar_moeda(t_si)}</td><td class="valores">{formatar_moeda(t_eop)}</td><td class="valores">{formatar_moeda(t_sop)}</td><td class="valores">{formatar_moeda(t_etr)}</td><td class="valores">{formatar_moeda(t_str)}</td><td class="valores valor-destaque">{formatar_moeda(t_sf)}</td><td class="valores">{formatar_moeda(t_lim)}</td></tr>'
    html_cons += '</tbody></table></div>'
