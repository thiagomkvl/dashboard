...
def carregar_dados(data_inicio, data_fim):
    ...
    try:
        ...
        except Exception as e: print("Aviso ao ler e processar Extratos_Tasy:", e)
        ...
        if df_extratos is not None and not df_extratos.empty:
            ...
            df_ext_caixa['Mes'] = df_ext_caixa['Data'].dt.to_period('M').dt.to_timestamp()
            df_extratos_mensal = df_ext_caixa.groupby('Mes').agg({'Vl Crédito': 'sum', 'Vl Débito': 'sum', 'Cred_Op': 'sum', 'Deb_Op': 'sum'}).reset_index()

            df_graficos = df_extratos_mensal.rename(columns={'Mes': 'Data'}).sort_values('Data').copy()
            ...
            df_graficos['Data_Label'] = df_graficos['Data'].dt.strftime('%m/%Y')
...
# ==============================================================================
periodo_str = f"{data_ini_painel.strftime('%m/%Y')} - {data_fim_painel.strftime('%m/%Y')}"
dt_ini_short = data_ini_painel.strftime('%m/%Y')
dt_fim_short = data_fim_painel.strftime('%m/%Y')
...
<div class="dashboard-header">
    ...
    <div class="header-center">
        ...
        <p>Controle Consolidado Mensal - Tasy</p>
    </div>
...
with col_diario:
    st.markdown(f"<div class='section-title'>SALDO MENSAL CONSOLIDADO</div>", unsafe_allow_html=True)
    
    if not df_graficos.empty:
        df_mensal_view = df_graficos.sort_values(by='Data', ascending=False)[['Data_Label', 'Saldo Inicial', 'Entrada Op', 'Saída Op', 'Saldo Final', 'Delta R$', 'Delta %']].copy()
    else:
        df_mensal_view = pd.DataFrame(columns=['Data_Label', 'Saldo Inicial', 'Entrada Op', 'Saída Op', 'Saldo Final', 'Delta R$', 'Delta %'])
    
    html_mensal = '<div class="tabela-container-scroll"><table class="tabela-financeira"><thead><tr>' \
    ...
    for _, row in df_mensal_view.iterrows():
        d_rs = row['Delta R$']
    ...
    html_mensal += '</tbody></table></div>'
    st.markdown(html_mensal, unsafe_allow_html=True)
