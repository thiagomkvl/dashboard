...
# ==============================================================================
# CUSTOM CSS — IDENTIDADE EXECUTIVA (BASEADA NA IMAGEM DE REFERÊNCIA)
# ==============================================================================
st.markdown("""
    <style>
    ...
    :root {
        --bg: #f2f5f9;
        --surface: #ffffff;
        --primary-dark: #002b66;  /* Azul Marinho Header */
        --primary: #2970d4;       /* Azul Principal */
        --success: #02b05c;       /* Verde Positivo */
        --danger: #fc4e51;        /* Vermelho Negativo */
        --text-main: #1a2332;
        --text-muted: #6b7785;
        --border: #d2dbe3;
    ...
    .update-badge span { font-size: 8px; color: #a4c2f4; }
...
# Filtros (Mockados no Header, Lógica no Script)
...
<div class="top-header">
    ...
    <div class="filters">
        <div class="filter-item">
            <label>Período</label>
            <div class="val">{nome_mes_atual} <span>▼</span></div>
        </div>
        <div class="filter-item">
            <label>Unidade</label>
            <div class="val">Todas <span>▼</span></div>
        </div>
        <div class="filter-item">
            <label>Centro de Custo</label>
            <div class="val">Todos <span>▼</span></div>
        </div>
...
# ==============================================================================
# 6. KPI CARDS (COM VARIAÇÃO MÊS ANTERIOR)
# ==============================================================================
def var_html(atual, ant, is_margin=False):
    if is_margin: val = atual - ant
    else: val = calc_var(atual, ant)
    ...
    <div class="kpi-card c-blue">
        <div class="kpi-content">
            ...
            <div class="kpi-text">
                ...
                <div class="kpi-val">R$ {formata_kpi(rec_bruta)}</div>
            </div>
        </div>
        <div class="kpi-meta">Mês Ant.: R$ {formata_kpi(ant_rec_bruta)}</div>
        {var_html(rec_bruta, ant_rec_bruta)}
    </div>
    ...
    <div class="kpi-card c-green">
        <div class="kpi-content">
            ...
            <div class="kpi-text">
                ...
                <div class="kpi-val">{formata_pct(mg_bruta)}</div>
            </div>
        </div>
        <div class="kpi-meta">Mês Ant.: {formata_pct(ant_mg_bruta)}</div>
        {var_html(mg_bruta, ant_mg_bruta, True)}
    </div>
    ...
    <div class="kpi-card c-green">
        <div class="kpi-content">
            ...
            <div class="kpi-text">
                ...
                <div class="kpi-val">R$ {formata_kpi(ebitda)}</div>
            </div>
        </div>
        <div class="kpi-meta">Mês Ant.: R$ {formata_kpi(ant_ebitda)}</div>
        {var_html(ebitda, ant_ebitda)}
    </div>
    ...
    <div class="kpi-card c-green">
        <div class="kpi-content">
            ...
            <div class="kpi-text">
                ...
                <div class="kpi-val">{formata_pct(mg_ebitda)}</div>
            </div>
        </div>
        <div class="kpi-meta">Mês Ant.: {formata_pct(ant_mg_ebitda)}</div>
        {var_html(mg_ebitda, ant_mg_ebitda, True)}
    </div>
    ...
    <div class="kpi-card c-blue">
        <div class="kpi-content">
            ...
            <div class="kpi-text">
                ...
                <div class="kpi-val">R$ {formata_kpi(lucro_liq)}</div>
            </div>
        </div>
        <div class="kpi-meta">Mês Ant.: R$ {formata_kpi(ant_lucro_liq)}</div>
        {var_html(lucro_liq, ant_lucro_liq)}
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_g1:
    ...
    x_water = ["Receita<br>Bruta", "Deduções", "Receita<br>Líquida", "CPV / CSP", "Lucro<br>Bruto", "OPEX", "EBITDA", "Depreciação<br>e Outros", "Lucro<br>Líquido"]
    ...
    fig_w = go.Figure(go.Waterfall(
        ...
        increasing={"marker": {"color": "#002b66"}}, # Azul Marinho para totais e positivos
        decreasing={"marker": {"color": "#ff4d4d"}}, # Vermelho para saídas
        totals={"marker": {"color": "#002b66"}}
    ))
    fig_w.update_layout(
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
        xaxis=dict(showgrid=False, tickfont=dict(size=10, color="#1a2332", weight="bold")),
...
with col_g2:
    ...
    # Prepara série histórica para o gráfico
    hist_meses = [pd.Period(m).strftime('%b').capitalize() for m in meses_str[-6:]] # Pega últimos 6 meses
    ...
    fig_c.update_layout(
        ...
        margin=dict(t=20, b=10, l=0, r=0), height=300, barmode='group'
    )
...
# Lado Esquerdo: Tabela DRE
html_tabela = f"""
<div class="table-container">
    <div class="table-header">
        <span>🧮</span> Matriz DRE Gerencial
    </div>
    <table class="dre-table">
        <thead>
            <tr>
                ...
                <th style="text-align:center;">AV %<br>(Receita Líq.)</th>
            </tr>
        </thead>
        <tbody>
"""
...
for nome, val_at, val_ant, destaque in linhas_dre:
    ...
    # Lógica de Cor (Despesas invertem a lógica do verde/vermelho)
    cor_var = "color: #02b05c;" if var_pct > 0 else "color: #fc4e51;"
    if "Custo" in nome or "Deduções" in nome or "Despesas" in nome:
...
# Lado Direito: Análise Gerencial Insights
# Geração dinâmica de texto baseado nos resultados
...
<div class="insights-container">
    ...
    <div class="insight-item">
        ...
        <div class="insight-text">
            ...
            <p>Variação de {calc_var(rec_bruta, ant_rec_bruta):.1f}% vs. mês anterior, indicando o ritmo de crescimento sustentável das operações.</p>
        </div>
    </div>
    ...
    <div class="insight-item">
        ...
        <div class="insight-text">
            ...
            <p>A Margem EBITDA fechou em {mg_ebitda:.1f}%, refletindo a eficiência da operação perante os custos variáveis e despesas fixas.</p>
        </div>
    </div>
    ...
    <div class="insight-item">
        ...
        <div class="insight-text">
            ...
            <p>Resultado de R$ {formata_kpi(lucro_liq)} no período, com variação de {calc_var(lucro_liq, ant_lucro_liq):.1f}% vs. último mês.</p>
        </div>
    </div>
    ...
    <div class="insight-item">
        ...
        <div class="insight-text">
            ...
            <p>Manter disciplina de custos operacionais (OPEX) e seguir com a estratégia de otimização de serviços prestados.</p>
        </div>
    </div>
</div>
</div> <!-- fecha bottom-grid -->
"""
