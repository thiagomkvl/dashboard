...
st.set_page_config(page_title="Dashboard Gerencial", layout="wide", page_icon="📊")

# --- BLOQUEIO DE SEGURANÇA (Para impedir acesso direto pelo link) ---
if not st.session_state.get("password_correct"):
    st.warning("🔒 Acesso restrito. Por favor, faça login na página inicial.")
    st.markdown("[Ir para Login](/)") # Link para a Home
    st.stop()
...
try:
    ...
    if not df_hist.empty:
        ...
        with c1:
            ...
            st.plotly_chart(px.pie(df_hoje, values='Saldo_Limpo', names='Classe ABC', hole=0.4), use_container_width=True)
        with c2:
